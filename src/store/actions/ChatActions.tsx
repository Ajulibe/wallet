...
const fetchChatListFeedSuccess: ActionCreator<ChatActionTypes> = (
   chats: ChatListInterface[]
) => {
   return { type: FETCH_CONTACT_CHAT_LIST, payload: chats };
};
const fetchChatSuccess: ActionCreator<ChatActionTypes> = (
   chats: ChatListInterface[]
) => {
...
export function fetchChat({
   ...
   return (dispatch: any) => {
      dispatch(loading());
      return chatService.fetchChat({ postId, userId }).then(
         (response) => {
