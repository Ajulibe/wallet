const COLORS = {
   light: {
      primary: "#12B36C",
      primaryLight: "rgba(16,179,108, 0.6)",
      secondary: "#FF5703", //#DB3C02
      secondaryLight: "rgba(221,74,10,0.2)",
      primaryDisabled: "rgba(71,204,175,0.4)",
      disabled: "rgba(112,112,112,0.4)",
      white: "#ffffff",
      black: "#000",
      tint: "#f7f7f7",
      transparent: "transparent",
      //Tags
      tagBgActive: "rgba(252,237,231,1)",
      tagTextInactive: "rgba(166,172,180,1)",
      // text
      textBlack: "#202F44",
      textBlackLight: "rgba(32,47,68,0.6)",
      textBlack50: "rgba(32,47,68,0.5)", //50% opacity
      textBlack40: "rgba(32,47,68,0.4)", //40% opacity
      tabBarInactive: "rgba(209,213,219,1)",
      red: "#EB5757",
      green: "#6FCF97",

      //input
      inputBorder: "rgba(32,47,68,0.15)",
      inputBorderError: "#E39092",
      inputPlaceholder: "rgba(166,172,180,1)",
      inputLabel: "#94A4B1",
      inputBg: "transparent",
      inputOtpBg: "#FCFEFF",
      inputBgError: "#FDFAFA",
      inputText: "rgba(32,47,68,1)",
      inputTextError: "rgba(126,126,126,.9)",
      fadedSecondary: "rgba(255,87,1,0.4)",
      lighterdisabled: "rgba(112,112,112,0.2)",
      //fund wallet medium list colors
      postAgentBgColor: "#3454D1",
      cardBgColor: "#FF5703",
      mobileBgColor: "#27AE60",
      bankAccBgColor: "#FFC526",
      ussdBgColor: "#9B51E0"
   },
   dark: {
      text: "#fff",
      background: "#000"
   }
};
export default COLORS;
